...
export default function App() {
  ...
  const [activeTab, setActiveTab] = useState<
    "Pipeline" | "Translator" | "LLM" | "RAG"
  >("Pipeline");

  const [pipelineMetrics, setPipelineMetrics] = useState<any | null>(null);
  const [activeOnnxFamily, setActiveOnnxFamily] = useState<"m2m" | "nllb">(
    "m2m",
  );
  ...
  async function fetchCurrentLlm() {
    try {
      ...
      appendLog(
        `LLM: Selected ${prettyBackendName(current)} | Loaded ${prettyBackendName(current)}`,
      );
    } catch (e: any) {
  ...
  async function loadModel(modelName: string) {
    try {
      ...
      appendLog(
        `LLM: Selected ${prettyBackendName(modelName)} | Loaded ${prettyBackendName(modelName)}`,
      );
    } catch (err) {
  ...
  async function refreshTranslatorModels() {
    try {
      const res =
        await axiosInstance.get<TranslatorStatusResponse>("/translator_status");
      ...
      appendLog(
        `Translator: Selected ${selectedTranslator} | Loaded ${loadedTranslator}`,
      );
    } catch (err: any) {
  ...
  async function loadTranslatorModel(
    id: "m2m_onnx" | "nllb_onnx" | "nllb_pytorch",
  ) {
    try {
      if (id === "nllb_pytorch") {
      ...
      appendLog(
        `Translator load error: ${String(err?.response?.data?.error || err?.message || err)}`,
      );
    }
  }
  ...
  async function unloadTranslatorModel() {
    try {
      ...
      appendLog(
        `Translator unload error: ${String(err?.response?.data?.error || err?.message || err)}`,
      );
    }
  }
  ...
  async function refreshRagModels() {
    try {
      ...
      const available: string[] = Array.isArray(res.data?.available)
        ? res.data.available
        : [];
      ...
      appendLog(
        `RAG: Selected ${prettyBackendName(active)} | Loaded ${prettyBackendName(loaded || "none")}`,
      );
    } catch (err: any) {
      appendLog(
        `RAG status error: ${String(err?.response?.data?.error || err?.message || err)}`,
      );
    }
  }
  ...
  async function loadRagModel(id: string) {
    try {
      ...
      appendLog(
        `RAG load error: ${String(err?.response?.data?.error || err?.message || err)}`,
      );
    }
  }
  ...
  async function unloadRagModel() {
    try {
      ...
      appendLog(
        `RAG unload error: ${String(err?.response?.data?.error || err?.message || err)}`,
      );
    }
  }
  ...
  async function sendUserMessage(text: string) {
    ...
    try {
      ...
      const res = await fetch(buildApiUrl("/infer"), {
        method: "POST",
        headers: {
