...
import axiosInstance, { buildApiUrl } from "../lib/axiosInstance";
...
export default function TranslatorPage() {
  ...
  const [metricsDataset, setMetricsDataset] = useState<"demo" | "flores200">(
    "demo",
  );
  ...
  const [translatorStatus, setTranslatorStatus] =
    useState<TranslatorStatus | null>(null);
  const [selectedBackend, setSelectedBackend] = useState<"onnx" | "nllb">(
    "onnx",
  );
  const [backendLoading, setBackendLoading] = useState<
    "m2m_onnx" | "nllb_onnx" | "nllb_pytorch" | null
  >(null);
  const [downloadingFamily, setDownloadingFamily] = useState<
    "m2m" | "nllb" | null
  >(null);
  const [downloadingFileKey, setDownloadingFileKey] = useState<string | null>(
    null,
  );
  const [downloadingTokenizerFamily, setDownloadingTokenizerFamily] = useState<
    "m2m" | "nllb" | null
  >(null);
  const [onnxCatalogStatus, setOnnxCatalogStatus] = useState<
    Record<"m2m" | "nllb", OnnxCatalogStatus | null>
  >({
    m2m: null,
    nllb: null,
  });
  ...
  const activeOnnxFamily =
    translatorStatus?.onnx?.family === "nllb" ? "nllb" : "m2m";
  ...
  async function fetchTranslatorStatus() {
    try {
      const res =
        await axiosInstance.get<TranslatorStatus>("/translator_status");
      setTranslatorStatus(res.data);
      const activeKey =
        res.data.active_translator_key ??
        (res.data.active_translator === "nllb" ? "nllb" : "onnx");
      setSelectedBackend(activeKey);
      setLogs((l) => [
        ...l,
        `Translator status: ${String(res.data.active_translator).toUpperCase()}`,
      ]);
  ...
  async function fetchOnnxDownloadStatus() {
    try {
      const [m2mRes, nllbRes] = await Promise.all([
        ...
        axiosInstance.get<OnnxCatalogStatus>(
          "/onnx_models/catalog?family=nllb",
        ),
      ]);
      ...
      setLogs((l) => [
        ...l,
        `Catalog status error: ${err?.response?.data?.error || err?.message || err}`,
      ]);
    }
  }

  async function loadTranslationBackend(
    target: "m2m_onnx" | "nllb_onnx" | "nllb_pytorch",
  ) {
    if (!translatorStatus || backendLoading) return;

    if (
      target === "m2m_onnx" &&
      !translatorStatus.onnx_families?.m2m?.available
    ) {
    ...
    if (
      target === "nllb_onnx" &&
      !translatorStatus.onnx_families?.nllb?.available
    ) {
    ...
    try {
      ...
      setLogs((l) => [
        ...l,
        `Load error: ${err?.response?.data?.error || err.message}`,
      ]);
    } finally {
  ...
  async function downloadOnnxModels(family: "m2m" | "nllb") {
    ...
    setLogs((l) => [
      ...l,
      `Downloading ${family.toUpperCase()} ONNX models...`,
    ]);
    ...
    try {
      ...
      const requested = Array.isArray(res.data?.requested_files)
        ? res.data.requested_files.length
        : 0;
      const downloaded = Array.isArray(res.data?.downloaded)
        ? res.data.downloaded.length
        : 0;
      const downloadedItems: Array<{ name?: string }> = Array.isArray(
        res.data?.downloaded,
      )
        ? res.data.downloaded
        : [];
      ...
      if (downloadedItems.length > 0) {
        ...
        const progressItems: string[] = Array.isArray(res.data?.progress)
          ? res.data.progress
          : [];
        const completionLines = progressItems
          .filter(
            (line) =>
              typeof line === "string" &&
              line.toLowerCase().startsWith("downloaded "),
          )
          .map(
            (line) =>
              `Downloaded model: ${line.replace(/^downloaded\s+/i, "").trim()}`,
          );
      ...
      setLogs((l) => [
        ...l,
        `Default ${family.toUpperCase()} ONNX models download complete (${downloaded}/${requested} files)`,
      ]);
      await fetchOnnxDownloadStatus();
      await fetchTranslatorStatus();
    } catch (err: any) {
      ...
      setLogs((l) => [
        ...l,
        `Download error: ${err?.response?.data?.error || err.message}`,
      ]);
    } finally {
  ...
  async function downloadSingleOnnxModel(
    family: "m2m" | "nllb",
    fileName: string,
  ) {
    ...
    setLogs((l) => [
      ...l,
      `Downloading ${fileName} (${family.toUpperCase()} family)...`,
    ]);
    ...
    try {
      ...
      setLogs((l) => [
        ...l,
        `File download error (${fileName}): ${err?.response?.data?.error || err.message}`,
      ]);
    } finally {
  ...
  async function downloadOnnxTokenizer(family: "m2m" | "nllb") {
    if (downloadingTokenizerFamily || downloadingFamily || downloadingFileKey)
      return;
    setDownloadingTokenizerFamily(family);
    setLogs((l) => [
      ...l,
      `Downloading tokenizer for ${family.toUpperCase()} ONNX...`,
    ]);

    try {
      const res = await axiosInstance.post(
        "/onnx_tokenizer/ensure",
        {
          family,
      ...
      setLogs((l) => [
        ...l,
        `Tokenizer download error (${family}): ${err?.response?.data?.error || err.message}`,
      ]);
    } finally {
  ...
  async function handleTranslate() {
    ...
    try {
      const res = await fetch(buildApiUrl("/translate"), {
        method: "POST",
        headers: {
      ...
      const processEvent = (eventData: string) => {
        try {
          ...
          if (payload.type === "meta") {
            ...
            setLogs((l) => [
              ...l,
              `Backend: ${(payload.backend || selectedBackend).toUpperCase()}`,
            ]);
      ...
      const msg =
        err?.name === "AbortError"
          ? `Request aborted: ${String(err?.message)}`
          : String(err);
  ...
  async function measureMetrics() {
    ...
    setLogs((l) => [
      ...l,
      `Measuring ${backend}${modelInfo} metrics for ${srcLang} → en...`,
    ]);
          ...
          <div className="mb-4 p-3 bg-blue-50 dark:bg-blue-900/20 rounded border border-blue-200 dark:border-blue-700">
            ...
            <div className="text-xs mb-3 space-y-1">
              <div className="flex items-center gap-2">
                ...
                {selectedBackend === "onnx" && activeOnnxFamily === "m2m" ? (
                  <span className="ml-auto px-2 py-0.5 rounded bg-green-600 text-white">
                    Loaded
                  </span>
                ) : (
                  <button
              ...
              <div className="flex items-center gap-2">
                <span>
                  NLLB PyTorch:{" "}
                  {translatorStatus.nllb.available
                    ? "✓ Available"
                    : "✗ Unavailable"}
                </span>
            ...
            <div className="mt-3 p-2 bg-white dark:bg-slate-800 rounded text-xs">
              ...
              {selectedBackend === "onnx" ? (
                <div className="text-gray-600 dark:text-gray-400 space-y-0.5">
                  <div>
                    Type:{" "}
                    {activeOnnxFamily === "m2m" ? "M2M ONNX" : "NLLB ONNX"}
                  </div>
                  <div>
                    Encoder file:{" "}
                    {translatorStatus.onnx.active_models?.encoder || "-"}
                  </div>
                  <div>
                    Decoder file:{" "}
                    {translatorStatus.onnx.active_models?.decoder || "-"}
                  </div>
                  <div>
                    LM head file:{" "}
                    {translatorStatus.onnx.active_models?.lm_head || "-"}
                  </div>
                </div>
              ) : (
                <div className="text-gray-600 dark:text-gray-400 space-y-0.5">
            ...
            <div className="mt-3 pt-3 border-t border-blue-200 dark:border-blue-700">
              ...
              {(["m2m", "nllb"] as const).map((family) => {
                ...
                const downloadedFamilyCount =
                  status?.downloaded_family_files ?? 0;
                const isFamilyComplete = Boolean(status?.all_family_downloaded);
                const isDefaultComplete = Boolean(
                  status?.all_default_downloaded,
                );
                const isBusy = downloadingFamily === family;
                const isTokenizerBusy = downloadingTokenizerFamily === family;
                const isTokenizerKnown =
                  typeof status?.tokenizer_ready === "boolean";
                ...
                return (
                  <div
                    key={family}
                    className="mb-2 p-2 rounded border bg-white dark:bg-slate-800"
                  >
                    <div className="flex items-center justify-between mb-1">
                      <div className="text-xs font-medium">{label}</div>
                      <span
                        className={`text-[10px] px-2 py-0.5 rounded ${isDefaultComplete ? "bg-green-100 text-green-700" : "bg-amber-100 text-amber-700"}`}
                      >
                        {isDefaultComplete
                          ? "Defaults downloaded"
                          : "Defaults missing"}
                    ...
                    <div className="text-[10px] text-slate-700 dark:text-slate-200 mb-2 p-2 rounded border border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-slate-900">
                      ...
                      <button
                        ...
                        disabled={
                          Boolean(downloadingFamily) ||
                          Boolean(downloadingFileKey) ||
                          Boolean(downloadingTokenizerFamily) ||
                          isTokenizerReady ||
                          !isTokenizerKnown
                        }
                        ...
                        {status.family_files_status.map((file) => (
                          ...
                          >
                            ...
                            {file.downloaded ? (
                              ...
                              <button
                                onClick={() =>
                                  downloadSingleOnnxModel(family, file.name)
                                }
                                disabled={
                                  Boolean(downloadingFamily) ||
                                  Boolean(downloadingFileKey)
                              ...
                              >
                                {downloadingFileKey === `${family}:${file.name}`
                                  ? "downloading..."
                                  : "download"}
                              </button>
                            )}
        ...
        <div className="mt-4 space-y-2">
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setMetricsDataset(e.target.value as "demo" | "flores200")
              }
              className="w-full px-2 py-1.5 text-sm rounded border bg-white dark:bg-slate-700"
            >
            ...
            <>
              <div>
                ...
                <select
                  value={floresSplit}
                  onChange={(e) =>
                    setFloresSplit(e.target.value as "dev" | "devtest")
                  }
                  className="w-full px-2 py-1.5 text-sm rounded border bg-white dark:bg-slate-700"
                >
              ...
              <div>
                ...
                <input
                  ...
                  value={floresSamples}
                  onChange={(e) =>
                    setFloresSamples(Math.max(1, Number(e.target.value || 1)))
                  }
                  className="w-full px-2 py-1.5 text-sm rounded border bg-white dark:bg-slate-700"
                />
              </div>
            ...
            translatorStatus?.onnx.active_models && (
              <div className="text-xs text-gray-600 dark:text-gray-400 p-2 bg-gray-100 dark:bg-gray-800 rounded">
                <div className="font-semibold mb-1">
                  {activeOnnxFamily === "m2m" ? "M2M ONNX" : "NLLB ONNX"} files
                  used for benchmark:
                </div>
                <div>
                  Encoder file: {translatorStatus.onnx.active_models.encoder}
                </div>
                <div>
                  Decoder file: {translatorStatus.onnx.active_models.decoder}
                </div>
                <div>
                  LM head file: {translatorStatus.onnx.active_models.lm_head}
                </div>
              </div>
            )}
      ...
      <main className="col-span-8 p-4 bg-white dark:bg-slate-800 rounded-lg shadow overflow-y-auto max-h-screen">
        ...
        {metrics && (
          <div className="mt-6">
            ...
            {metrics.dataset?.name === "FLORES-200" ||
            typeof metrics.quality?.corpus_bleu === "number" ? (
              <div className="grid grid-cols-2 gap-4">
                <div className="p-4 bg-slate-800 border border-slate-700 rounded-lg text-white">
                  ...
                  <div className="text-sm space-y-1">
                    ...
                    <div>
                      ...
                      {metrics.dataset?.split || floresSplit}
                    </div>
                    <div>
                      <span className="font-medium">Samples:</span>{" "}
                      {metrics.dataset?.samples_evaluated ?? floresSamples}
                    </div>
                    <div>
                      <span className="font-medium">Lang:</span>{" "}
                      {metrics.dataset?.src_lang || srcLang} →{" "}
                      {metrics.dataset?.tgt_lang || "en"}
                    </div>
                    <div>
                      <span className="font-medium">Backend:</span>{" "}
                      {String(
                        metrics.dataset?.backend || backend,
                      ).toUpperCase()}
                    </div>
                  </div>
                </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    <div>
                      ...
                      <div className="text-2xl font-bold text-yellow-700">
                        {Number(metrics.quality?.corpus_bleu ?? 0).toFixed(2)}
                      </div>
                    </div>
                    <div>
                      <div className="font-medium">Corpus chrF</div>
                      <div className="text-2xl font-bold text-yellow-700">
                        {Number(metrics.quality?.corpus_chrf ?? 0).toFixed(2)}
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="col-span-2 p-4 bg-green-50 dark:bg-green-900/20 rounded-lg">
                  ...
                  <div className="grid grid-cols-4 gap-3 text-sm">
                    <div>
                      <span className="font-medium">Avg / sentence:</span>
                      <br />
                      {Number(
                        metrics.throughput?.avg_time_per_sentence_s ?? 0,
                      ).toFixed(4)}
                      s
                    </div>
                    <div>
                      <span className="font-medium">Chars / sec:</span>
                      <br />
                      {Number(
                        metrics.throughput?.forward?.chars_per_sec ??
                          metrics.throughput?.chars_per_sec ??
                          0,
                      ).toFixed(2)}
                    </div>
                    <div>
                      ...
                      <br />
                      {Number(
                        metrics.throughput?.forward?.tokens_per_sec ??
                    ...
                    <div>
                      ...
                      <br />
                      {Number(metrics.throughput?.total_time_s ?? 0).toFixed(3)}
                      s
                    </div>
                  </div>
                </div>

                {Array.isArray(metrics.examples) &&
                    ...
                    <div className="col-span-2 p-4 bg-slate-50 dark:bg-slate-900/50 rounded-lg">
                      ...
                      <div className="space-y-3 text-sm max-h-80 overflow-y-auto">
                        {metrics.examples.slice(0, 5).map((ex, idx) => (
                          <div
                            key={idx}
                            className="p-2 rounded bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700"
                          >
              ...
              metrics.throughput?.forward &&
              metrics.throughput?.roundtrip &&
              metrics.quality &&
              metrics.memory ? (
              <div className="grid grid-cols-2 gap-4">
                {/* Input Section */}
                <div className="p-4 bg-slate-800 border border-slate-700 rounded-lg text-white">
                  ...
                  <div className="text-sm space-y-1">
                    <div>
                      <span className="font-medium">Backend:</span>{" "}
                      {metrics.input.backend?.toUpperCase() || "N/A"}
                    </div>

                    {/* Show ONNX models used if backend is ONNX */}
                    {(metrics.input.backend === "onnx" ||
                      metrics.input.backend === "m2m_onnx" ||
                      metrics.input.backend === "nllb_onnx") &&
                      translatorStatus?.onnx.active_models && (
                        <div className="mt-2 p-2 bg-blue-100 dark:bg-blue-800/30 rounded text-xs">
                          <div className="font-semibold mb-1">
                            {activeOnnxFamily === "m2m"
                              ? "M2M ONNX"
                              : "NLLB ONNX"}{" "}
                            files used:
                          </div>
                    ...
                    <div>
                      ...
                      {metrics.input.char_length}
                    </div>
                    <div>
                      <span className="font-medium">Tokens (est):</span>{" "}
                      {metrics.input.token_length_estimate}
                    </div>
                  ...
                  <div className="text-sm space-y-2">
                    ...
                    <div className="p-2 bg-green-100 dark:bg-green-800/30 rounded">
                      ...
                      <div className="text-xs text-green-600 dark:text-green-400 mt-1">
                        {metrics.throughput.forward.tokens_per_sec.toFixed(1)}{" "}
                        tok/s •{" "}
                        {metrics.throughput.forward.chars_per_sec.toFixed(1)}{" "}
                        char/s
                      </div>
                    </div>
                    ...
                    <div className="p-2 bg-green-100 dark:bg-green-800/30 rounded">
                      ...
                      <div className="text-xs text-green-600 dark:text-green-400 mt-1">
                        {metrics.throughput.roundtrip.tokens_per_sec.toFixed(1)}{" "}
                        tok/s •{" "}
                        {metrics.throughput.roundtrip.chars_per_sec.toFixed(1)}{" "}
                        char/s
                      </div>
                    </div>
                    ...
                    <div className="mt-2 pt-2 border-t border-green-200 dark:border-green-700 text-xs">
                      <div>
                        ...
                        {metrics.end_to_end_time_s.toFixed(2)}s
                      </div>
                      <div className="text-[10px] text-green-600 dark:text-green-400">
                        Includes quality metrics (BLEU/CHRF) calculation
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="p-4 bg-purple-50 dark:bg-purple-900/20 rounded-lg">
                  ...
                  <div className="text-sm space-y-1">
                    <div>
                      ...
                      {metrics.memory.baseline_rss_mb.toFixed(2)} MB
                    </div>
                    <div>
                      <span className="font-medium">Peak:</span>{" "}
                      {metrics.memory.peak_rss_mb.toFixed(2)} MB
                    </div>
                    <div>
                      <span className="font-medium">Peak Increase:</span>{" "}
                      {metrics.memory.peak_increase_mb.toFixed(2)} MB
                    </div>
                    <div>
                      <span className="font-medium">After Forward:</span>{" "}
                      {metrics.memory.after_forward_rss_mb.toFixed(2)} MB
                    </div>
                    <div>
                      <span className="font-medium">Translation Δ:</span>{" "}
                      {metrics.memory.translation_increase_mb.toFixed(2)} MB
                    </div>
                  </div>
                </div>
                  ...
                  <div className="p-4 bg-orange-50 dark:bg-orange-900/20 rounded-lg">
                    ...
                    <div className="text-sm space-y-1">
                      <div>
                        ...
                        {metrics.vram.total_mb.toFixed(2)} MB
                      </div>
                      <div>
                        <span className="font-medium">Baseline:</span>{" "}
                        {metrics.vram.baseline_used_mb.toFixed(2)} MB
                      </div>
                      <div>
                        <span className="font-medium">Peak Used:</span>{" "}
                        {metrics.vram.peak_used_mb.toFixed(2)} MB
                      </div>
                      <div>
                        <span className="font-medium">After Forward:</span>{" "}
                        {metrics.vram.after_forward_used_mb.toFixed(2)} MB
                      </div>
                      <div className="mt-2 w-full bg-gray-200 rounded-full h-2.5 dark:bg-gray-700">
                        <div
                          className="bg-orange-600 h-2.5 rounded-full"
                          style={{
                            width: `${((metrics.vram.peak_used_mb / metrics.vram.total_mb) * 100).toFixed(1)}%`,
                          }}
                        ></div>
                      </div>
                      <div className="text-xs text-center">
                        {(
                          (metrics.vram.peak_used_mb / metrics.vram.total_mb) *
                          100
                        ).toFixed(1)}
                        % utilized
                      </div>
                ...
                <div className="col-span-2 p-4 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg">
                  ...
                  <div className="grid grid-cols-3 gap-4 text-sm">
                    <div>
                      ...
                      <div className="text-2xl font-bold text-yellow-700">
                        {metrics.quality.bleu_score.toFixed(2)}
                      </div>
                    </div>
                    <div>
                      <div className="font-medium">chrF Score</div>
                      <div className="text-2xl font-bold text-yellow-700">
                        {metrics.quality.chrf_score.toFixed(2)}
                      </div>
                    </div>
                    <div>
                      <div className="font-medium">Length Similarity</div>
                      <div className="text-2xl font-bold text-yellow-700">
                        {metrics.quality.char_length_similarity_pct.toFixed(1)}%
                      </div>
                    </div>
                  </div>
                  <div className="mt-3 grid grid-cols-2 gap-2 text-xs">
                    <div>
                      ...
                      <div>
                        {metrics.quality.forward_output_tokens} tokens,{" "}
                        {metrics.quality.forward_output_chars} chars
                      </div>
                    </div>
                    <div>
                      <div className="font-medium">Roundtrip Output:</div>
                      <div>
                        {metrics.quality.roundtrip_output_tokens} tokens,{" "}
                        {metrics.quality.roundtrip_output_chars} chars
                      </div>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="space-y-3">
                    <div>
                      <div className="text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">
                        Forward ({metrics.input.src_lang} →{" "}
                        {metrics.input.tgt_lang}):
                      </div>
                    ...
                    <div>
                      <div className="text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">
                        Roundtrip ({metrics.input.tgt_lang} →{" "}
                        {metrics.input.src_lang}):
                      </div>
                      <div className="p-2 bg-white dark:bg-slate-800 rounded text-sm">
              ...
              <div className="p-4 bg-red-50 dark:bg-red-900/20 rounded-lg text-sm text-red-700 dark:text-red-300">
                Metrics response is missing expected fields. Try measuring again
                or check backend response format.
              </div>
            )}
          </div>
        )}
